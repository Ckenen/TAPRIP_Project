...
# For stranded-specific Paired-end RNA-seq.
...
awk '{if(($4 ~ /\/1$/ && $6 == "+") || ($4~ /\/2$/ && $6 == "-")){print $0}}' ${prefix}.bed > ${prefix}_pos.bed
awk '{if(($4 ~ /\/1$/ && $6 == "-") || ($4~ /\/2$/ && $6 == "+")){print $0}}' ${prefix}.bed > ${prefix}_neg.bed
...
bedtools genomecov -bg -split -i ${prefix}_pos.bed -g ${prefix}_genome_size.txt | sort -k1,1 -k2,2n > ${prefix}_raw_pos.bedGraph
bedGraphToBigWig ${prefix}_raw_pos.bedGraph ${prefix}_genome_size.txt ${prefix}_raw_pos.bw

bedtools genomecov -bg -split -i ${prefix}_neg.bed -g ${prefix}_genome_size.txt | sort -k1,1 -k2,2n > ${prefix}_raw_neg.bedGraph
...
bedtools genomecov -bg -split -scale ${scale} -i ${prefix}_pos.bed -g ${prefix}_genome_size.txt | sort -k1,1 -k2,2n > ${prefix}_norm_pos.bedGraph
bedGraphToBigWig ${prefix}_norm_pos.bedGraph ${prefix}_genome_size.txt ${prefix}_norm_pos.bw

bedtools genomecov -bg -split -scale ${scale} -i ${prefix}_neg.bed -g ${prefix}_genome_size.txt | sort -k1,1 -k2,2n > ${prefix}_norm_neg.bedGraph
